    ...
    public void checkUsrID(String ID, HashMap<String,String> ids, ActionEvent event) throws IOException, InterruptedException {
        if(ids.containsKey(ID)){ // if the id exists in the hashmap
            ...
            if(actor == "Student"){
                ...
                StudentController passID =  studentPage.getController();
                passID.setId(usrID.getText());
            ...
            if(actor.equals("Agent")) {
                ...
                Thread.sleep(100);
                // do the student view here!
                FXMLLoader agentPage = new FXMLLoader(getClass().getResource("/resources/AgentView.fxml"));
                root = agentPage.load();
               // AgentController agent = agentPage.getController();
               // agent.displayOrders();
                stage = (Stage)((Node)event.getSource()).getScene().getWindow();
                stage.setTitle("Welcome || Agent");
                scene = new Scene(root);
                stage.setScene(scene);
                stage.show();
